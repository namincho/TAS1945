...
namespace Tas1945_mon
{
	public partial class MainForm : Form
	{
				...
				for (int i = 0; i < iPixelDataSize; i++)
				{
					...
					else // 테두리 한줄씩은 위의 'if'와 'else if'에서 죽인 후 else 진입
					{
						...
						else
						{
							...
							int judge = 0;
							float horizontal_gradient = 0, vertical_gradient = 0;
							float ret = 0; // 임시 변수

							judge = Active[row - 1, col] + Active[row, col - 1] + Active[row, col + 1] + Active[row + 1, col];

							if (judge < 2)
							{
								// Neighbor Pixel 4개 중에 2개 이상 살아있지 않은 경우에는 원본값(DEAD)그대로 유지
								asPixelData[i] = asPixelData[i];
								Active_ISP[row, col] = 0;
							}
							else
							{
								horizontal_gradient = (float)Math.Abs(Image_buf_disp[row, col - 1] - Image_buf_disp[row, col + 1]);
								vertical_gradient = (float)Math.Abs(Image_buf_disp[row - 1, col] - Image_buf_disp[row + 1, col]);

								if (horizontal_gradient > vertical_gradient)
									ret = (float)((Image_buf_disp[row - 1, col] + Image_buf_disp[row + 1, col]) / 2);
								else if (horizontal_gradient < vertical_gradient)
									ret = (float)((Image_buf_disp[row, col - 1] + Image_buf_disp[row, col + 1]) / 2);
								else
									ret = (float)((Image_buf_disp[row - 1, col] + Image_buf_disp[row + 1, col]
										+ Image_buf_disp[row, col - 1] + Image_buf_disp[row, col + 1]) / 4);
				...
				for (int i = 0; i < iPixelDataSize; i++)
				{
					...
					else // 테두리 한줄씩은 위의 'if'와 'else if'에서 죽인 후 else 진입
					{
						...
						else
						{
							...
							judge = Active_ISP[row - 1, col] + Active_ISP[row, col - 1] + Active_ISP[row, col + 1] + Active_ISP[row + 1, col];
		...
		public void Sensitivity_Cal(ref float[] asPixelData, int iPixelDataSize)
        {
			...
			int temp_debugMode = 1;   // 0 : new ,  1 : old
